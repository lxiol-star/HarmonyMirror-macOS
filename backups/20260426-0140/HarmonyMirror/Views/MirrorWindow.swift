...
struct MirrorWindow: View {
    ...
    private func targetContentSize(for window: NSWindow?) -> CGSize {
        ...
        let maxVideoHeight = max(280, maxContentHeight - toolbarHeight - statusHeight)
        let maxWidth = min(visibleFrame.width * 0.94, source.width >= source.height ? 1040 : 560)
        let maxHeight = min(maxVideoHeight, source.width >= source.height ? 640 : maxVideoHeight)
    ...
    private func resizeWindowIfNeeded() {
        ...
        guard abs(current.width - target.width) > 8 || abs(current.height - target.height) > 8 else {
