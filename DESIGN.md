...
- 手机示例：`Display ID: 0` 为 `1316×2832`，桌面状态下左上区域下拉打开通知栏，右上区域下拉打开控制中心。
- 平板示例：`Display ID: 0` 为 `1600×2560`（竖屏）/`2560×1600`（横屏），可用于初始化窗口尺寸和横竖屏自动切换。
...
- **手动输入**：在"设备 IP"输入框填写 `IP:端口`（如 `192.168.1.100:10178`），点击"连接 Wi-Fi"
...
- [x] **InputInjector Agent 优先路径** — Agent 可用时优先发送 TouchFrame，不可用时自动回退 hdc
- [x] **mac 触控板手势适配**
  - 双指滑动作为滚动输入
  - 双指张开 / 捏合用于缩放窗口或画面
